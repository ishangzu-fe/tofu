<template>
    <div class="el-select"
        ...
        <div class="el-select-tags" v-if="multiple" @click.stop="toggleMenu" ref="tags" :style="{ 'max-width': inputWidth - 32 + 'px' }">
            <transition-group @after-leave="resetInputHeight">
                <i-tag v-for="item in selected"
                        ...
                        close-transition>{{ item.currentLabel }}</i-tag>
            </transition-group>
            <input type="text"
        ...
        <transition name="md-fade-bottom" @after-leave="doDestroy">
            <i-select-menu ref="popper" v-show="visible && emptyText !== false">
                <ul class="el-select-dropdown-list" v-show="options.length > 0 && filteredOptionsCount > 0 && !loading">
                    <slot></slot>
                </ul>
                <p class="el-select-dropdown-empty" v-if="emptyText">{{ emptyText }}</p>
            </i-select-menu>
        </transition>
    </div>
</template>
...
<script type="text/babel">
    ...
    export default {
        ...
        computed: {
            iconClass() {
                return this.showCloseIcon ? 'circle-close' : (this.remote && this.filterable ? '' : 'arrow-up');
            },
