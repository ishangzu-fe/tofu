  ...
  <div
    ...
    <transition
      ...
      <el-select-menu
        ...
        <p class="el-select-dropdown-empty" v-if="emptyText && (allowCreate && options.length === 0 || !allowCreate)">{{ emptyText }}</p>
      </el-select-menu>
    </transition>
  </div>
</template>
...
<script type="text/babel">
  import Emitter from './dependencies/mixins/emitter';
  import Locale from './dependencies/mixins/locale';
  import ElInput from './dependencies/packages/input';
  ...
  import Clickoutside from './dependencies/utils/clickoutside';
  import { addClass, removeClass, hasClass } from './dependencies/utils/dom';
  import { addResizeListener, removeResizeListener } from './dependencies/utils/resize-event';
  import { t } from './dependencies/locale';
  const sizeMap = {
    'large': 42,
    'small': 30,
    'mini': 22
  ...
  export default {
    ...
    methods: {
      ...
      handleMenuEnter() {
        if (!this.dropdownUl) {
          this.dropdownUl = this.$refs.popper.$el.querySelector('.el-select-dropdown-wrap');
          this.getOverflows();
        }
